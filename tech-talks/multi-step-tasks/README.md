# Multi-Step Tasks: When One Agent Needs to Become Many
...
**Use multi-step tasks when:**
- Your task requires distinct phases (research → analysis → implementation)
- You need to explore multiple approaches without cluttering main context
- Different parts need focused investigation without cross-contamination
- You want parallel execution of independent research or analysis tasks

**Don't use multi-step tasks when:**
- The task is simple enough for one agent in one go (use Plan Mode instead)
- You need specialized roles like separate planners/reviewers (see [agent-teams](../agent-teams/))
- You need agents working on different branches simultaneously (see [parallel-execution](../parallel-execution/))

### Quick Decision Guide

```
Q: Is your task too complex for one agent?
...
└─ Yes →
    ...
    └─ No →
        ...
        └─ No → 👉 Use multi-step-tasks (you are here)
```
...
- **Sequential bottlenecks**
  Complex research, analysis, and implementation tasks execute one-at-a-time without isolation
...
- Detailed implementation work with isolated errors
...
### Specialized Analysis with Custom Agents

**The problem:** Different analysis concerns require different expertise and focus.
...
**Better:** Delegate coherent, meaningful work units that benefit from isolation.
...
**Problem:** Subagents research but main agent doesn't synthesize findings.
...
### 🔧 Control
Fine-grained visibility and restriction controls enable sophisticated workflows
